...
<div class="address">
    ...
    <c:if test="${category.CATE_PARENT_ID==0}">
    <span>/</span><a href="selectproduct?id=${category.CATE_ID }">${category.CATE_NAME}</a>
    </c:if>
    <c:if test="${category.CATE_PARENT_ID!=0}">
     <span>/</span><a href="selectproduct?id=${fcategory.CATE_ID }" class="on">${fcategory.CATE_NAME}</a>
     <span>/</span><a href="selectproduct?id=${category.CATE_ID }" class="on">${category.CATE_NAME}</a>
    </c:if>
    </div>
</div><!-------------------current---------------------->
<div class="current">
...
<c:if test="${p.PRODUCT_FCATE==category.CATE_ID }">
	<li><a href="proDetail.html">
        <dl>
            <dt><img src="/upload/${p.PRODUCT_FILENAME}"></dt>
            <dd>${p.PRODUCT_DESCRIPTION }</dd>
...
<c:if test="${category.CATE_PARENT_ID!=0 }">
<c:forEach var="p" items="${plist }">
<c:if test="${p.PRODUCT_CCATE==category.CATE_ID }">
	<li><a href="proDetail.html">
        <dl>
            <dt><img src="/upload/${p.PRODUCT_FILENAME}"></dt>
            <dd>${p.PRODUCT_DESCRIPTION }</dd>
